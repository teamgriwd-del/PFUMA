...
root holds the code (`src/`, `app/`, `app-webview/`, `backend/`, `public/`), its build/config
files, `run_project.bat`, and the main `README.md`.

> **Scope: PFUMA/INGCEBO is software only.** There is no hardware or IoT in the product. Don't
> mention, pitch or budget hardware in any document. Old hardware files still in the repo are
> legacy and out of scope; see the root `CLAUDE.md`.
...
| [`business-and-funding/`](business-and-funding/) | Funding and procurement papers, including the TelOne commercialisation request. New files here are git-ignored because they are internal. |
| [`posters-and-diagrams/`](posters-and-diagrams/) | The technical and public posters: HTML sources, PNG previews, final PDFs, and the Python scripts that generate them. |
...
1. **[PLATFORM_OVERVIEW.md](platform/PLATFORM_OVERVIEW.md)** — what PFUMA/INGCEBO is, the problem it
   solves, and current build status.
...
- The platform docs describe the codebase as of **2026-09-24**. Where something here conflicts with
