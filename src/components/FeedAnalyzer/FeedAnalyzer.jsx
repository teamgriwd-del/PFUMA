  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
        <div>
          <label className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2 block">Build a ration for</label>
        ...
        {requirement && (
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-gray-50 rounded-xl p-3 text-center">
              ...
              <p className="text-sm font-bold text-gray-900">{requirement.weight_kg} kg</p>
            </div>
            <div className="bg-gray-50 rounded-xl p-3 text-center">
              <Clock size={14} className="mx-auto text-gray-400 mb-1" />
              <p className="text-xs font-bold text-gray-400 uppercase">Life Stage</p>
              <p className="text-sm font-bold text-gray-900 capitalize">{requirement.life_stage}</p>
            </div>
            <div className="bg-gray-50 rounded-xl p-3 text-center">
              <ShieldCheck size={14} className="mx-auto text-gray-400 mb-1" />
              <p className="text-xs font-bold text-gray-400 uppercase">Daily Target</p>
              <p className="text-sm font-bold text-gray-900">{requirement.target_protein_g}g CP</p>
            </div>
          </div>
        )}
      </div>
      ...
      {requirement && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          ...
          <button onClick={addRow} className="flex items-center gap-1.5 text-xs font-bold text-pfuma-green hover:underline">
            ...
            <div className="pt-4 border-t border-gray-50 space-y-4">
                  ...
                  return (
                    <div key={n.label} className={`rounded-xl border p-3 ${s.bg}`}>
                      <div className="flex items-center gap-1.5 mb-1">
                        ...
                        <span className={`text-xs font-bold uppercase ${s.color}`}>{n.label} — {s.label}</span>
                      </div>
              ...
              <div className="flex items-center justify-between bg-gray-900 rounded-xl p-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-white uppercase tracking-wide">Estimated Daily Cost</span>
                </div>
              ...
              <button
                onClick={savePlan}
                disabled={saving}
                className="w-full py-3 bg-pfuma-green text-white rounded-xl text-xs font-bold uppercase tracking-wide hover:brightness-105 disabled:opacity-50 transition"
              >
              ...
              {savedPlan && (
                <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl p-3">
                  ...
                  <p className="text-xs font-bold text-green-700">Saved — added USD {savedPlan.total_cost_usd.toFixed(2)} to {selectedAnimal?.name}'s cost-to-date.</p>
                </div>
              )}
            </div>
          )}
      ...
      {history.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Past Ration Plans for {selectedAnimal?.name}</p>
              ...
              return (
                <div key={p.id} className="flex items-center justify-between text-xs bg-gray-50 rounded-xl p-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-gray-800">USD {Number(p.total_cost_usd).toFixed(2)}</span>
                  </div>
                </div>
              );
            })}
        ...
        <div className="flex gap-2 shrink-0 flex-wrap">
          {SPECIES.map(s => (
            <button key={s} onClick={() => setSpecies(s)}
              className={`px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide transition border ${species === s ? 'bg-pfuma-green text-white border-pfuma-green shadow-md' : 'bg-white text-gray-500 border-gray-200 hover:border-pfuma-green'}`}>
              {s}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div key={feed.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full uppercase ${CAT_COLOR[feed.category] || CAT_COLOR.mixed}`}>
                        {feed.category}
                      </span>
                      {feed.market_price && (
                        <span className="text-xs font-bold px-2 py-0.5 rounded-full uppercase bg-green-100 text-green-700">
                          ${Number(feed.market_price.price).toFixed(2)}/{feed.market_price.unit} live
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-xs text-gray-500 font-medium">Protein: <span className="font-bold text-blue-600">{feed.protein_percent}%</span></span>
                      <span className="text-gray-300">·</span>
                      <span className="text-xs text-gray-500 font-medium">Energy: <span className="font-bold text-orange-600">{feed.energy_mj} MJ/kg</span></span>
                      {species_list.length > 0 && (
                        <>
                          <span className="text-gray-300">·</span>
                          <span className="text-xs text-gray-400 font-medium">{species_list.slice(0,3).join(', ')}{species_list.length > 3 ? '...' : ''}</span>
                        </>
                      )}
                    </div>
                  </div>
                ...
                {isOpen && (
                  <div className="px-5 pb-5 border-t border-gray-50 pt-4 space-y-4">
                    {feed.description && (
                      <p className="text-xs text-gray-600 font-medium leading-relaxed bg-orange-50 border border-orange-100 rounded-xl p-3">
                        {feed.description}
                      </p>
                    )}
                    <div>
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Nutritional Breakdown</p>
                      <div className="space-y-2.5">
                    ...
                    {species_list.length > 0 && (
                      <div>
                        <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Suitable For</p>
                        <div className="flex gap-2 flex-wrap">
                          {species_list.map(s => (
                            <span key={s} className="text-xs font-bold bg-pfuma-green/10 text-pfuma-green px-3 py-1 rounded-full uppercase">
                              {s}
                            </span>
                          ))}
                        </div>
          ...
          {filtered.length === 0 && !loading && (
            <div className="col-span-2 bg-white border-2 border-dashed border-gray-200 rounded-2xl py-12 text-center">
              ...
              <p className="text-sm font-bold text-gray-400">No feeds match your search</p>
    ...
    <div className="bg-ivory min-h-full text-left overflow-y-auto">
      ...
      <Hero
        ...
        aside={
          <span className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-bold backdrop-blur-sm ${
            apiOnline
              ? 'border-green-300/40 bg-green-950/40 text-green-50'
              : 'border-amber-300/40 bg-amber-950/40 text-amber-50'
          }`}>
            {apiOnline
              ? <><CheckCircle size={14} aria-hidden="true" /> Live data — {feeds.length} feed types</>
              : <><AlertTriangle size={14} aria-hidden="true" /> Demo — start the Flask API for live data</>}
          </span>
        }
      />
      ...
      <div className="flex gap-2 bg-white p-1.5 rounded-2xl border border-gray-100 shadow-sm w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide transition ${tab === 'ration' ? 'bg-pfuma-green text-white shadow' : 'text-gray-500 hover:text-gray-700'}`}
        >
