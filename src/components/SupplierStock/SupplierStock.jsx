    ...
    <div className={`p-4 rounded-2xl border-2 ${outOfStock ? 'bg-red-50 border-red-200' : lowStock ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-100'}`}>
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="min-w-0">
          <p className="text-xs font-bold text-gray-800 truncate">{listing.product_name}</p>
          <p className="text-xs text-gray-400 font-medium uppercase mt-0.5">{listing.category}</p>
        </div>
        <span className={`text-xs font-bold px-2 py-1 rounded-full uppercase shrink-0 ${STATUS_BADGE[listing.status]?.cls || 'bg-gray-100 text-gray-600'}`}>
          {STATUS_BADGE[listing.status]?.label || listing.status}
        </span>
      </div>
      <div className="flex items-center gap-2 mb-2">
        {outOfStock ? (
          <span className="flex items-center gap-1 text-xs font-bold text-red-600"><AlertTriangle size={12} /> Out of stock</span>
        ) : (
          <span className={`text-sm font-bold ${lowStock ? 'text-amber-700' : 'text-gray-800'}`}>{Number(listing.quantity).toLocaleString()} {listing.unit} left</span>
        )}
        {lowStock && !outOfStock && <span className="flex items-center gap-1 text-xs font-bold text-amber-600 uppercase"><AlertTriangle size={10} /> Low stock</span>}
      </div>
      <form onSubmit={submit} className="flex gap-2">
        ...
        />
        <button type="submit" disabled={busy || !adding} className="shrink-0 flex items-center gap-1 px-3 py-2 bg-gray-800 text-white rounded-lg font-bold text-xs uppercase hover:bg-gray-900 transition disabled:opacity-50">
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-full space-y-6 text-left overflow-y-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2"><Package size={20} className="text-pfuma-gold" /> Supply Chain</h2>
          <p className="text-xs text-gray-400 font-medium mt-1">Real stock levels for what you sell — restock here as orders come in.</p>
        </div>
        <button onClick={() => setActiveTab('marketplace')} className="flex items-center gap-2 px-4 py-2.5 bg-pfuma-gold text-white rounded-xl text-xs font-bold uppercase hover:bg-amber-600 transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-gray-100 rounded-2xl p-5">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-1">Active Products</p>
          <p className="text-3xl font-bold text-gray-900">{listings.length}</p>
        </div>
        <div className={`border rounded-2xl p-5 ${lowStockCount ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-100'}`}>
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-1">Low Stock</p>
          <p className={`text-3xl font-bold ${lowStockCount ? 'text-amber-700' : 'text-gray-900'}`}>{lowStockCount}</p>
        </div>
        <div className={`border rounded-2xl p-5 ${outOfStockCount ? 'bg-red-50 border-red-200' : 'bg-white border-gray-100'}`}>
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-1">Out of Stock</p>
          <p className={`text-3xl font-bold ${outOfStockCount ? 'text-red-700' : 'text-gray-900'}`}>{outOfStockCount}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border-2 border-dashed border-gray-200 rounded-2xl bg-white">
          ...
          <p className="text-sm font-bold text-gray-400">No products posted yet</p>
          <p className="text-xs text-gray-400 font-medium mt-1">Post medicine, equipment or feed on the Marketplace to start selling</p>
        </div>
      ) : (
      ...
      <button onClick={load} className="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase hover:text-gray-600 transition mx-auto">
