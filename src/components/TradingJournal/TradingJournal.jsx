        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
              <div className="flex justify-between items-start mb-2">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wide">Total Trades</p>
                <TrendingUp size={16} className="text-pfuma-green" />
              </div>
              <p className="text-3xl font-bold text-gray-900">{totals?.total_trades ?? 0}</p>
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
              ...
              <p className="text-xs text-gray-400 font-medium mt-1">Across every completed trade</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="text-sm font-bold text-gray-800">{counterpartyLabel}</h3>
            </div>
            <p className="text-xs text-gray-400 font-medium mb-4">Your top trading partners, ranked by {isSupplier ? 'order count' : 'value traded'}</p>
            {counterparties.length === 0 ? (
              <div className="text-center py-10 border-2 border-dashed border-gray-200 rounded-2xl">
                <BookOpen size={28} className="mx-auto text-gray-300 mb-2" />
                <p className="text-sm font-bold text-gray-400">No completed trades yet</p>
              </div>
            ) : (
              <div className="space-y-2.5">
                {counterparties.map((cp, i) => (
                  <div key={cp.counterparty_id} className="flex items-center gap-3 p-3.5 bg-gray-50 rounded-xl">
                    <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center font-bold text-pfuma-green text-xs shrink-0 shadow-sm">#{i + 1}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-gray-800 truncate">{cp.counterparty_name}</p>
                      <p className="text-xs text-gray-400 font-medium">{cp.trade_count} trade{cp.trade_count !== 1 ? 's' : ''}</p>
                    </div>
                    <p className="text-sm font-bold text-pfuma-green shrink-0">
