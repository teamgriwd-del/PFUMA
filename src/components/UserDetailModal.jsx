      ...
      <div className="bg-white w-full max-w-2xl max-h-[85vh] overflow-y-auto overflow-x-hidden rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-white border-b border-gray-100 px-4 sm:px-6 py-4 flex items-center justify-between gap-2 z-10">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center text-white font-bold text-xs shrink-0 overflow-hidden">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-400 font-bold uppercase truncate">{user.role} · Account #{user.id}</p>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-6 space-y-6 min-w-0">
          ...
          {idMatches === undefined ? (
            <div className="rounded-xl bg-gray-50 border border-gray-100 px-3 py-2.5">
              <p className="text-xs text-gray-400 font-medium italic">Checking national ID against other accounts…</p>
            </div>
          ) : idMatches.length > 0 ? (
            <div className="rounded-xl bg-red-50 border border-red-200 px-3 py-2.5">
              <div className="flex items-center gap-2 mb-1.5">
                <AlertTriangle size={14} className="text-red-600 shrink-0" />
                <p className="text-xs font-bold text-red-700 uppercase tracking-wide">Same National ID on {idMatches.length} other account{idMatches.length !== 1 ? 's' : ''}</p>
              </div>
              <div className="space-y-1">
                {idMatches.map(m => (
                  <p key={m.id} className="text-xs text-red-700 font-medium">
            ...
            <div className="rounded-xl bg-green-50 border border-green-200 px-3 py-2 flex items-center gap-2">
              ...
              <p className="text-xs text-green-700 font-bold">No other account shares this National ID.</p>
            </div>
          )}

          <section>
            <h4 className="text-xs font-bold text-pfuma-green uppercase tracking-wide mb-1 pb-1 border-b border-gray-100">Identity</h4>
          ...
          <section>
            <h4 className="text-xs font-bold text-pfuma-green uppercase tracking-wide mb-1 pb-1 border-b border-gray-100">Location & Organisation</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 min-w-0">
          ...
          {(user.farm_size_ha || user.species_farmed) && (
            <section>
              <h4 className="text-xs font-bold text-pfuma-green uppercase tracking-wide mb-1 pb-1 border-b border-gray-100">Farmer Details</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 min-w-0">
          ...
          {(user.license_number || user.speciality) && (
            <section>
              <h4 className="text-xs font-bold text-pfuma-green uppercase tracking-wide mb-1 pb-1 border-b border-gray-100">Veterinary Credentials</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 min-w-0">
          ...
          {(user.business_reg || user.supply_categories || user.trading_areas) && (
            <section>
              <h4 className="text-xs font-bold text-pfuma-green uppercase tracking-wide mb-1 pb-1 border-b border-gray-100">Business Details</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 min-w-0">
          ...
          {user.institution_type && (
            <section>
              <h4 className="text-xs font-bold text-pfuma-green uppercase tracking-wide mb-1 pb-1 border-b border-gray-100">Institution Details</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 min-w-0">
          ...
          {(user.badge_number || user.station) && (
            <section>
              <h4 className="text-xs font-bold text-pfuma-green uppercase tracking-wide mb-1 pb-1 border-b border-gray-100">Police Details</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 min-w-0">
          ...
          <section>
            <h4 className="text-xs font-bold text-pfuma-green uppercase tracking-wide mb-1 pb-1 border-b border-gray-100">Verification & Account Status</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 min-w-0">
            ...
            <div className="flex gap-2 mt-2">
              {user.id_document_path && (
                <button onClick={() => viewDocument('id')} disabled={docBusy === 'id'} className="flex items-center gap-1.5 px-3 py-2 bg-gray-50 hover:bg-gray-100 rounded-lg text-xs font-bold uppercase text-gray-600 transition disabled:opacity-50">
                  <FileText size={12} /> {docBusy === 'id' ? 'Opening…' : 'View ID Document'}
                </button>
              )}
              {user.credential_document_path && (
                <button onClick={() => viewDocument('credential')} disabled={docBusy === 'credential'} className="flex items-center gap-1.5 px-3 py-2 bg-gray-50 hover:bg-gray-100 rounded-lg text-xs font-bold uppercase text-gray-600 transition disabled:opacity-50">
          ...
          {user.next_of_kin_name && (
            <section>
              <h4 className="text-xs font-bold text-pfuma-green uppercase tracking-wide mb-1 pb-1 border-b border-gray-100">Next of Kin</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 min-w-0">
