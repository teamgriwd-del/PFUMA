...
const CATEGORY_META = {
  'Physical Signs':       { tip: 'Look at the skin, mouth, legs, eyes, and lymph nodes.' },
  'Respiratory':          { tip: 'Listen to breathing and watch for nasal discharge or coughing.' },
  'Behavior & Vitals':    { tip: 'Changes in eating, movement, temperature, and alertness.' },
};
...
const StepBadge = ({ n, label, active, done }) => (
  <div className={`flex items-center gap-2.5 px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap shrink-0 transition-colors
    ${done  ? 'bg-green-100 text-green-800'
    : active ? 'bg-bark-500 text-white'
    :          'bg-cream text-gray-500'}`}>
    <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[0.625rem] font-bold
      ${done ? 'bg-green-600 text-white' : active ? 'bg-white/25 text-white' : 'bg-white text-gray-400'}`}>
      {done ? '✓' : n}
    </span>
    {label}
  </div>
...
const ConfidenceExplainer = ({ value, matched, total }) => (
  <div className="mt-2">
    <div className="flex justify-between text-xs font-bold text-gray-400 mb-1">
      ...
      <span className="font-bold text-gray-700">{value}% match</span>
    </div>
  ...
  return (
    <div className="bg-ivory min-h-full text-left">
      ...
      <Hero
        ...
        eyebrow="AI disease checker · 10-disease knowledge base"
        title="Symptom to diagnosis in three steps"
        sub="Select every symptom you can see on the animal. The checker scores them against 10 known livestock diseases and names the most likely match — with a step-by-step action plan and prevention guide."
      />

      {/* Progress strip — its own full-width band under the hero rather than
          an aside inside it: three labelled steps plus connectors is far too
          wide to sit beside a headline without crushing the type. */}
      <div className="bg-white border-b border-bark-500/10">
        <div className="px-6 xl:px-8 py-4 flex items-center gap-2 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <StepBadge n="1" label="Pick animal"   active={!step1done} done={step1done} />
          <ArrowRight size={14} className="text-gray-300 shrink-0" aria-hidden="true" />
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className={`bg-white border-2 rounded-2xl p-5 transition ${step1done ? 'border-pfuma-green' : 'border-gray-100'}`}>
            <div className="flex items-center gap-2 mb-1">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step1done ? 'bg-pfuma-green text-white' : 'bg-gray-100 text-gray-500'}`}>{step1done ? '✓' : '1'}</div>
              <h3 className="text-sm font-bold text-gray-800">Which animal are you examining?</h3>
              <span className="text-xs text-gray-400 font-medium ml-1">(optional — lets you save the result to its record)</span>
            </div>
            {animals.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold uppercase border-2 transition ${!targetAnimalId ? 'bg-gray-900 text-white border-gray-900' : 'bg-gray-50 text-gray-500 border-gray-100 hover:border-gray-300'}`}
                >
                  General Check
                </button>
                {animals.map(a => (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold uppercase border-2 transition ${String(targetAnimalId) === String(a.id) ? 'bg-pfuma-green text-white border-pfuma-green shadow-md' : 'bg-gray-50 text-gray-500 border-gray-100 hover:border-pfuma-green'}`}
                  >
          ...
          <div className="bg-white border-2 border-gray-100 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-1">
              <div className="w-6 h-6 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center text-xs font-bold"></div>
              <h3 className="text-sm font-bold text-gray-800">Photo Check <span className="text-gray-400 font-medium text-xs">(Optional shortcut)</span></h3>
            </div>
            <p className="text-xs text-gray-400 font-medium mb-4">Upload a clear photo of the animal's skin, mouth, or legs. We'll scan it for visible discoloration and texture patterns and suggest symptoms to check — this is a coarse visual heuristic, not a diagnosis, so always verify against what you see in person.</p>
            ...
            {isAnalyzingImage ? (
              <div className="flex items-center gap-4 py-4">
                ...
                <div>
                  <p className="text-sm font-bold text-pfuma-green">Scanning photo...</p>
                  <p className="text-xs text-gray-400 font-medium">Reading pixel colour and texture patterns</p>
                </div>
              </div>
            ) : visualResult ? (
              <div className="flex items-start gap-4 bg-green-50 border border-green-200 rounded-2xl p-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs font-bold bg-pfuma-green text-white px-2 py-0.5 rounded-full uppercase">Photo Scan · Heuristic</span>
                  </div>
                  <p className="text-sm font-bold text-gray-800">{visualResult.headline}</p>
                  ...
                  <button onClick={() => setVisualResult(null)} className="mt-2 text-xs font-bold text-gray-400 hover:text-red-500 uppercase underline">Remove photo</button>
                </div>
              </div>
            ) : (
              <label className="flex items-center gap-4 cursor-pointer group border-2 border-dashed border-gray-200 hover:border-pfuma-green rounded-2xl p-4 transition" aria-label="Upload animal photo">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 font-medium">Best results: skin, mouth, hooves, or swollen areas · JPG, PNG</p>
                </div>
              </label>
            )}
          </div>
          ...
          <div className="bg-white border-2 border-gray-100 rounded-2xl p-5">
            <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step2done ? 'bg-pfuma-green text-white' : 'bg-gray-100 text-gray-500'}`}>{step2done ? '✓' : '2'}</div>
                  <h3 className="text-sm font-bold text-gray-800">Tick every symptom you can see</h3>
                </div>
                <p className="text-xs text-gray-400 font-medium ml-8">Check all symptoms that apply — more symptoms = more accurate result. Ticked symptoms turn green.</p>
              </div>
              {selectedSymptoms.length > 0 && (
                <div className="flex items-center gap-2 bg-pfuma-green/10 border border-pfuma-green/20 px-3 py-1.5 rounded-full">
                  ...
                  <span className="text-xs font-bold text-pfuma-green">{selectedSymptoms.length} symptom{selectedSymptoms.length !== 1 ? 's' : ''} selected</span>
                </div>
              )}
            </div>
                ...
                return (
                  <div key={cat.name}>
                    <div className="flex items-center gap-2 mb-2 pb-2 border-b border-gray-50">
                      <h4 className="text-xs font-bold text-gray-600 uppercase tracking-wide">{cat.name}</h4>
                      {catSelected > 0 && (
                        <span className="ml-auto text-xs font-bold text-pfuma-green bg-green-50 px-2 py-0.5 rounded-full">{catSelected} selected</span>
                      )}
                    </div>
                    {meta.tip && <p className="text-xs text-gray-400 font-medium mb-3">{meta.tip}</p>}
            ...
            <div className="flex gap-3 mt-6 pt-5 border-t border-gray-50">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-4 bg-pfuma-green text-white rounded-2xl font-bold uppercase text-xs tracking-wide shadow-lg shadow-green-900/20 hover:bg-green-700 transition disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <Stethoscope size={16} />
                {hasAnalyzed ? 'Re-run Analysis' : 'Run Diagnosis'}
              </button>
              {(selectedSymptoms.length > 0 || hasAnalyzed) && (
                <button onClick={resetAll} className="flex items-center gap-1.5 px-5 py-4 bg-gray-100 text-gray-500 rounded-2xl font-bold uppercase text-xs tracking-wide hover:bg-gray-200 transition">
          ...
          {hasAnalyzed && (
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full bg-pfuma-green text-white flex items-center justify-center text-xs font-bold">3</div>
                <h3 className="text-sm font-bold text-gray-800">Diagnosis Results</h3>
                <span className="text-xs text-gray-400 font-medium">— {diagnosisResults.length} possible match{diagnosisResults.length !== 1 ? 'es' : ''} found</span>
              </div>

              {savedFeedback && (
                <div className="flex items-center gap-2 bg-green-50 border border-green-200 text-green-800 text-xs font-bold px-4 py-3 rounded-xl" role="status">
              ...
              {diagnosisResults.length === 0 ? (
                <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center">
                  ...
                  <p className="text-sm font-bold text-gray-400">No disease matched above 20% confidence.</p>
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex items-center gap-2 flex-wrap">
                        {i === 0 && <span className="text-xs font-bold bg-pfuma-green text-white px-2 py-0.5 rounded-full uppercase">Top Match</span>}
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full uppercase ${severityBadge(res.severity)}`}>{res.severity}</span>
                        {res.quarantineRequired && (
                          <span className="text-xs font-bold bg-orange-100 text-orange-700 border border-orange-200 px-2 py-0.5 rounded-full uppercase flex items-center gap-1">
                            <AlertTriangle size={9} /> Quarantine
                          </span>
                        )}
                      </div>
                      <span className="text-xs text-gray-400 font-medium shrink-0">#{i + 1} result</span>
                    </div>

                    <h4 className="text-lg font-bold text-gray-900 mb-1">{res.name}</h4>
                    ...
                    {i > 0 && (
                      <button
                        className="mt-3 flex items-center gap-1 text-xs font-bold text-gray-400 hover:text-pfuma-green transition uppercase tracking-wide"
                    ...
                    <div className="border-t border-gray-100 p-5 space-y-4">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            {targetAnimalId && (
                              <button
                                ...
                                className="px-3 py-1.5 bg-pfuma-green text-white text-xs font-bold rounded-lg hover:bg-green-700 transition uppercase"
                              >
                                Save
                              </button>
                            )}
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`flex-1 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide transition ${activeTab === 'action' ? 'bg-pfuma-green text-white shadow-sm' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
                        >
                          Action Plan
                        </button>
                        <button
                          onClick={() => setActiveTab('prevention')}
                          className={`flex-1 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide transition ${activeTab === 'prevention' ? 'bg-pfuma-green text-white shadow-sm' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
                        >
                      ...
                      {res.severity === 'Critical' && (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <p className="text-xs font-bold uppercase tracking-wide">Call your vet immediately</p>
                            <p className="text-xs font-medium opacity-80">This is a critical disease — do not wait. Tap to message a vet now, or call DVS directly: +263 242 706331</p>
                          </div>
        ...
        <div className="col-span-1 space-y-5">
          ...
          <div className="bg-gray-900 rounded-2xl p-5 relative overflow-hidden">
            <div className="relative z-10">
              <h4 className="text-sm font-bold text-white mb-1">Diseases Near Your Farm</h4>
              <p className="text-xs text-gray-500 font-medium mb-4 leading-snug">
                ...
                {REGIONAL_THREATS.map(t => (
                  <div key={t.id} className={`flex items-center justify-between p-3 rounded-xl border ${levelColor(t.level)} bg-opacity-10 transition`}>
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="text-xs text-white font-bold leading-none">{t.disease}</p>
                        <p className="text-xs text-gray-500 font-medium">{t.district}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className={`text-xs font-bold uppercase block ${t.level === 'Critical' ? 'text-red-500' : t.level === 'High' ? 'text-orange-400' : 'text-yellow-400'}`}>{t.level}</span>
                      <span className="text-xs text-gray-600 font-bold">{t.dist}</span>
                    </div>
                  </div>
                ))}
              </div>
              <p className="text-xs text-gray-600 font-medium mt-4 leading-snug italic">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5">
            <h4 className="text-sm font-bold text-gray-800 mb-1 flex items-center gap-2"><Info size={14} className="text-pfuma-green" /> How the match % works</h4>
            <div className="space-y-3 mt-3">
              {[
                ...
                <div key={r.range} className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-gray-700">{r.range} — {r.label}</p>
          ...
          {!hasAnalyzed && (
            <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center">
              ...
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-1">Results will appear here</p>
              <p className="text-xs text-gray-300 font-medium">Tick symptoms on the left, then press "Run Diagnosis"</p>
            </div>
          )}

          {/* Quick tip */}
          <div className="bg-pfuma-green/5 border border-pfuma-green/20 rounded-2xl p-4">
            <p className="text-xs font-bold text-pfuma-green uppercase tracking-wide mb-1">Pro Tip</p>
