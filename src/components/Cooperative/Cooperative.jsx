    ...
    <div className="p-6 bg-pfuma-cream min-h-full text-left">
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="bg-gray-900 rounded-2xl p-6 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold text-yellow-400 uppercase tracking-[3px]">Cooperative</span>
            </div>
            <h2 className="text-2xl font-bold text-white leading-tight mb-1">Your Dip Tank or Grazing Group</h2>
        ...
        <div className="flex gap-1 bg-white border border-gray-100 rounded-2xl p-1 shadow-sm">
          <button onClick={() => setMode('browse')} className={`flex-1 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide transition ${mode === 'browse' ? 'bg-pfuma-green text-white' : 'text-gray-400 hover:text-gray-600'}`}>
            <Search size={13} className="inline mr-1.5 -mt-0.5" /> Find Mine
          </button>
          <button onClick={() => setMode('create')} className={`flex-1 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide transition ${mode === 'create' ? 'bg-pfuma-green text-white' : 'text-gray-400 hover:text-gray-600'}`}>
        ...
        {mode === 'browse' ? (
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-gray-800 mb-1">Cooperatives in {currentUser?.province || 'your province'}</h3>
            <p className="text-xs text-gray-400 font-medium mb-4">If your dip tank association is already here, join it — don't start a duplicate.</p>
            {loading ? (
              <p className="text-xs text-gray-400 font-medium italic text-center py-6">Loading…</p>
            ) : browseList.length === 0 ? (
              <div className="text-center py-10 border-2 border-dashed border-gray-200 rounded-2xl">
                <Users size={28} className="mx-auto text-gray-300 mb-2" />
                <p className="text-sm font-bold text-gray-400">None found yet</p>
                <p className="text-xs text-gray-400 font-medium mt-1">Be the first to start one for your area.</p>
              </div>
            ) : (
                ...
                {browseList.map(c => (
                  <div key={c.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 font-medium mt-0.5 flex items-center gap-1"><MapPin size={11} /> {c.district ? `${c.district}, ` : ''}{c.province} · {c.member_count} member{c.member_count !== 1 ? 's' : ''}</p>
                      {c.dip_tank_location && <p className="text-xs text-gray-400 font-medium mt-0.5">{c.dip_tank_location}</p>}
                    </div>
                    <button onClick={() => join(c.id)} disabled={busy} className="shrink-0 px-4 py-2 bg-pfuma-green text-white rounded-xl text-xs font-bold uppercase hover:bg-green-700 transition disabled:opacity-50">
                      Join
                    </button>
                  </div>
                ))}
          ...
          <form onSubmit={create} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm space-y-3">
            <h3 className="text-sm font-bold text-gray-800 mb-1">Start a new cooperative</h3>
            <div>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Dip Tank / Grazing Location</label>
            ...
            <p className="text-xs text-gray-400 font-medium">Province is set from your profile: {currentUser?.province || 'not set'}.</p>
            <button type="submit" disabled={busy} className="w-full py-3.5 bg-pfuma-green text-white rounded-xl font-bold uppercase text-xs tracking-wide hover:bg-green-700 transition disabled:opacity-50">
  ...
  return (
    <div className="p-6 bg-pfuma-cream min-h-full space-y-6 text-left overflow-y-auto h-full">
      <div className="bg-gray-900 rounded-2xl p-6 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div className="flex-1">
            ...
            <h2 className="text-2xl font-bold text-white leading-tight mb-1">{coop.name}</h2>
          ...
          <button onClick={leave} disabled={busy} className="shrink-0 flex items-center gap-1.5 px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs font-bold uppercase transition disabled:opacity-50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Members */}
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          <h3 className="text-sm font-bold text-gray-800 mb-1 flex items-center gap-2"><Users size={15} className="text-pfuma-green" /> Members</h3>
          <p className="text-xs text-gray-400 font-medium mb-3">Everyone sharing this dip tank / grazing group, and how many animals each of them has registered.</p>
          <div className="space-y-2.5">
            {coop.members.map(m => (
              <div key={m.id} className="flex items-center gap-3 p-2.5 bg-gray-50 rounded-xl">
                <div className="w-8 h-8 rounded-full bg-pfuma-green flex items-center justify-center text-white font-bold text-xs shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 font-medium">{m.animal_count} animal{m.animal_count !== 1 ? 's' : ''}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-1">
            <h3 className="text-sm font-bold text-gray-800 flex items-center gap-2"><Calendar size={15} className="text-blue-500" /> Dip Schedule</h3>
            {isAdmin && <button onClick={() => setShowDipForm(p => !p)} className="text-xs font-bold text-pfuma-green hover:underline uppercase">+ Add</button>}
          </div>
          ...
          {showDipForm && (
            <form onSubmit={submitDip} className="mb-4 space-y-2 bg-gray-50 rounded-xl p-3">
              ...
              <input placeholder="Notes (optional)" value={dipForm.notes} onChange={e => setDipForm(p => ({ ...p, notes: e.target.value }))}
                className="w-full p-2.5 bg-white rounded-lg border-2 border-transparent focus:border-pfuma-green outline-none text-xs font-bold" />
              <button type="submit" disabled={busy} className="w-full py-2 bg-pfuma-green text-white rounded-lg text-xs font-bold uppercase hover:bg-green-700 transition disabled:opacity-50">Save</button>
            </form>
          )}
          {dipSchedule.length === 0 ? (
            <p className="text-xs text-gray-400 font-medium italic text-center py-4">No dip day scheduled yet.</p>
          ) : (
            <div className="space-y-2.5">
              {dipSchedule.map(d => (
                <div key={d.id} className="p-3 bg-blue-50 border border-blue-100 rounded-xl">
                  <p className="text-xs font-bold text-gray-800">{new Date(d.scheduled_date).toDateString()}</p>
                  {d.notes && <p className="text-xs text-gray-500 font-medium mt-0.5">{d.notes}</p>}
                  <p className="text-xs text-gray-400 font-bold uppercase mt-1">Set by {d.created_by_name}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-1">
            <h3 className="text-sm font-bold text-gray-800 flex items-center gap-2"><Stethoscope size={15} className="text-pfuma-gold" /> Vet Requests</h3>
            <button onClick={() => setShowReqForm(p => !p)} className="text-xs font-bold text-pfuma-green hover:underline uppercase">+ Request</button>
          </div>
          <p className="text-xs text-gray-400 font-medium mb-3">Post once for the whole group — any verified vet in {coop.province} can claim it and see it as "open" until they do.</p>
          {showReqForm && (
            <form onSubmit={submitRequest} className="mb-4 space-y-2 bg-gray-50 rounded-xl p-3">
              ...
              <input type="date" min={new Date().toISOString().slice(0, 10)} value={reqForm.preferred_date} onChange={e => setReqForm(p => ({ ...p, preferred_date: e.target.value }))}
                className="w-full p-2.5 bg-white rounded-lg border-2 border-transparent focus:border-pfuma-green outline-none text-xs font-bold" />
              <button type="submit" disabled={busy} className="w-full py-2 bg-pfuma-green text-white rounded-lg text-xs font-bold uppercase hover:bg-green-700 transition disabled:opacity-50">Post</button>
            </form>
          )}
          {vetRequests.length === 0 ? (
            <p className="text-xs text-gray-400 font-medium italic text-center py-4">No vet requests yet.</p>
          ) : (
            <div className="space-y-2.5">
              {vetRequests.map(r => (
                <div key={r.id} className="p-3 bg-gray-50 rounded-xl">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <p className="text-xs font-bold text-gray-800 leading-snug">{r.reason}</p>
                    <span className={`shrink-0 text-xs font-bold px-2 py-0.5 rounded-full uppercase ${STATUS_BADGE[r.status]}`}>{r.status}</span>
                  </div>
                  <p className="text-xs text-gray-400 font-medium">By {r.requested_by_name}{r.preferred_date ? ` · wants ${new Date(r.preferred_date).toDateString()}` : ''}</p>
                  {r.vet_name && <p className="text-xs text-gray-400 font-medium">Vet: {r.vet_name}</p>}
                </div>
              ))}
            </div>
          )}
