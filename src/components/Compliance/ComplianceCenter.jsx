...
const StageChip = ({ stage }) => {
  ...
  return (
    <span className={`px-2 py-1 rounded-lg text-xs font-bold uppercase tracking-wider ${tone.chip}`}>
      {meta.short}
    </span>
  );
};
...
const CaseHeader = ({ c }) => (
  <div className="flex items-start gap-3 min-w-0">
    ...
    <div className="min-w-0">
      <p className="font-bold text-gray-800 text-sm leading-tight">{c.vaccine_name}</p>
...
const BlockerForm = ({ caseId, currentUser, onDone, onCancel }) => {
  ...
  return (
    <form onSubmit={submit} className="mt-3 p-4 bg-white rounded-2xl border border-gray-200">
      <p className="text-xs font-bold text-gray-800 mb-1">What is stopping you?</p>
      <p className="text-xs text-gray-500 font-medium mb-3 leading-snug">
        ...
        {BLOCKERS.map(b => (
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block text-xs font-bold text-gray-800">{b.label}</span>
              <span className="block text-xs text-gray-500 font-medium leading-snug">{b.hint}</span>
            </span>
          </label>
        ))}
      </div>
      ...
      {picked && (
        <p className="mt-3 flex items-start gap-1.5 text-xs font-bold text-blue-700 bg-blue-50 border border-blue-200 rounded-xl p-2.5">
      ...
      <div className="flex gap-2 mt-3">
        <button
          ...
          className="flex-1 bg-pfuma-green text-white text-xs font-bold uppercase tracking-wider py-3 rounded-xl disabled:opacity-50 flex items-center justify-center gap-2"
        >
          {busy && <Loader2 size={13} className="animate-spin" />}
          Send this to my vet
        </button>
        <button type="button" onClick={onCancel} className="px-4 text-xs font-bold uppercase tracking-wider text-gray-400">
          Cancel
        </button>
      </div>
    </form>
...
const FarmerCase = ({ c, currentUser, onChanged }) => {
  ...
  return (
    <div className={`p-4 rounded-2xl border ${tone.card}`}>
      ...
      <p className={`text-xs font-bold mt-3 leading-snug ${tone.text}`}>{meta.farmer}</p>
      ...
      {c.stage === 'deferred' && pauseLeft !== null && (
        <p className="mt-2 flex items-center gap-1.5 text-xs font-bold text-blue-700">
      ...
      {c.stage === 'penalty' && (
        <div className="mt-3 p-3 bg-white rounded-xl border border-red-200">
          <p className="flex items-center gap-1.5 text-xs font-bold text-red-600 mb-1">
      ...
      {flash && <p className="mt-3 text-xs font-bold text-pfuma-green bg-green-50 border border-green-200 rounded-xl p-2.5">{flash}</p>}

      {!showBlocker && !['resolved', 'waived'].includes(c.stage) && (
        <div className="flex flex-wrap gap-2 mt-3">
          <button
            onClick={() => setShowBlocker(true)}
            className="px-3.5 py-2 bg-white border border-bark-500/20 rounded-xl text-xs font-semibold text-gray-700 hover:border-bark-500/60 hover:text-bark-500 transition"
          >
            I can't vaccinate — here's why
          </button>
          <span className="px-3 py-2 text-xs font-bold text-gray-400 self-center">
...
const VetCase = ({ c, currentUser, onChanged }) => {
  ...
  return (
    <div className={`p-4 rounded-2xl border ${tone.card}`}>
      ...
      <p className={`text-xs font-bold mt-2 leading-snug ${tone.text}`}>{meta.vet}</p>

      {c.blocker_reason && (
        <div className="mt-3 p-3 bg-white rounded-xl border border-blue-200">
          <p className="flex items-center gap-1.5 text-xs font-bold text-blue-700 mb-1">
            <HandHelping size={12} /> Farmer reported: {BLOCKER_LABEL[c.blocker_reason]}
          </p>
          {c.blocker_notes && <p className="text-xs text-gray-600 font-medium leading-snug">"{c.blocker_notes}"</p>}
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {!c.vet_id && (
              <button onClick={() => act('claim')} disabled={busy}
                className="px-3 py-2 bg-white border-2 border-gray-200 rounded-xl text-xs font-bold uppercase tracking-wider text-gray-600 hover:border-pfuma-green disabled:opacity-50">
                Take this on
              </button>
            )}
            {c.blocker_reason && (
              <>
                <button onClick={() => act('accept_deferral')} disabled={busy}
                  className="px-3 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider disabled:opacity-50">
                  Accept blocker — pause
                </button>
                <button onClick={() => act('reject_deferral')} disabled={busy}
                  className="px-3 py-2 bg-white border-2 border-gray-200 rounded-xl text-xs font-bold uppercase tracking-wider text-gray-600 disabled:opacity-50">
                  Reject blocker
                </button>
              </>
            )}
            {['reminder', 'vet_followup', 'deferred'].includes(c.stage) && (
              <button onClick={() => act('issue_notice')} disabled={busy}
                className="px-3 py-2 bg-red-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider disabled:opacity-50 flex items-center gap-1.5">
                <FileWarning size={12} /> Issue formal notice
              </button>
            )}
            {lockoutReady && (
              <button onClick={() => act('apply_lockout', { blocker_reviewed: !c.blocker_reason })} disabled={busy}
                className="px-3 py-2 bg-gray-900 text-white rounded-xl text-xs font-bold uppercase tracking-wider disabled:opacity-50 flex items-center gap-1.5">
            ...
            {c.stage === 'penalty' && !c.conditional_clearance && (
              <button onClick={() => act('conditional_clearance')} disabled={busy}
                className="px-3 py-2 bg-white border-2 border-gray-200 rounded-xl text-xs font-bold uppercase tracking-wider text-gray-600 disabled:opacity-50 flex items-center gap-1.5">
                <Unlock size={12} /> Allow sale with vaccination at point of sale
              </button>
            )}
            <button onClick={() => act('waive')} disabled={busy}
              className="px-3 py-2 text-xs font-bold uppercase tracking-wider text-gray-400 hover:text-gray-600 disabled:opacity-50">
              Doesn't apply — waive
            </button>
          </div>
        </>
...
const ComplianceCenter = ({ currentUser }) => {
  ...
  const Stat = ({ icon: Icon, label, value, tone }) => (
    <div className={`p-4 rounded-2xl border ${TONE[tone].card}`}>
      ...
      <p className={`text-2xl font-bold mt-2 leading-none ${TONE[tone].text}`}>{value}</p>
  ...
  return (
    <div className="bg-ivory min-h-full text-left">
      ...
      <Hero
        size="sm"
        image={photo('barnPasture', { w: 1800, q: 74 })}
        eyebrow="Vaccination compliance"
        title={isVet ? 'Follow-up queue' : 'Your vaccination follow-ups'}
        sub={isVet
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {!isVet && (
          <div className="p-4 bg-white rounded-2xl border border-gray-100">
            <p className="text-xs font-bold uppercase tracking-wide text-gray-400 mb-2">How this works</p>
            <ol className="space-y-1.5 text-xs font-medium text-gray-600">
            ...
            <p className="mt-2.5 text-xs font-bold text-pfuma-green leading-snug">
        ...
        {!isVet && cases.some(c => c.stage === 'resolved') && (
          <p className="flex items-center justify-center gap-1.5 text-xs font-bold text-pfuma-green">
