      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {form.category === 'livestock' && (
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Which Animal? *</label>
              {listableAnimals.length > 0 ? (
                ...
                <p className="text-xs text-gray-500 font-medium p-3 bg-gray-50 rounded-xl">Register an animal in your Herd first — livestock listings must be linked to one.</p>
              )}
              {blockedCount > 0 && (
        ...
        {form.category === 'livestock' && form.animal_id && (
          <>
            <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-xs text-red-700 font-medium flex items-start gap-2">
            ...
            <div className="border border-gray-200 rounded-xl p-3.5 space-y-3">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Traditional Authority Clearance *</label>
                <p className="text-xs text-gray-500 mb-2">In communal areas the village head (Sabuku) or chief (Mambo) clears a sale before the police do. If you farm on a title deed or lease with no traditional authority, say so instead.</p>
                <div className="grid grid-cols-2 gap-2">
                  {[['attested', 'Cleared by Sabuku / Mambo'], ['not_applicable', 'Does not apply to my farm']].map(([val, lab]) => (
                    <button
                      ...
                      className={`text-xs font-bold rounded-lg px-3 py-2 border transition ${
              ...
              {form.leader_clearance === 'attested' && (
                <div className="space-y-2.5">
                  <div className="grid grid-cols-2 gap-2.5">
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Who cleared it *</label>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Their name *</label>
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Village / Ward</label>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Date cleared</label>
                      <input className={inputCls} type="date" value={form.leader_cleared_on} onChange={e => set('leader_cleared_on', e.target.value)} />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Reference on the written clearance</label>
              ...
              {form.leader_clearance === 'not_applicable' && (
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Why it does not apply *</label>
                  ...
                  <p className="text-xs text-gray-400 mt-1.5">The officer reviewing your listing will see this.</p>
                </div>
              )}
            </div>
            ...
            <div className="border border-gray-200 rounded-xl p-3.5 space-y-3">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Livestock Clearance Certificate (ZRP Form 392)</label>
                <p className="text-xs text-gray-500">Optional now — fill in what you know. The rest can be added later once a buyer is arranged, or left for the officer to complete on-site.</p>
              </div>
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Livestock Register No.</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Dip Tank / Farm No.</label>
                  <input className={inputCls} type="text" value={form.dip_tank_name} onChange={e => set('dip_tank_name', e.target.value)} />
                </div>
              </div>

              <button type="button" onClick={() => setShowBuyerFields(s => !s)} className="text-xs font-bold text-pfuma-green hover:underline uppercase tracking-wide">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Animal Description (colour, marks, etc.)</label>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Your Signature (Seller/Owner)</label>
        ...
        <div>
          <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Product / Item Name *</label>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Price (USD) *</label>
          ...
          <div>
            ...
            {form.category !== 'livestock' && (
              <p className="text-xs text-gray-400 font-medium mt-1">How many {form.unit} you have on hand — this is your stock. It drops as farmers order, and you can top it back up any time from Supply Chain.</p>
            )}
          </div>
        </div>
        <div>
        ...
        <div>
          <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Photo (optional)</label>
          <label htmlFor="listing-photo" className="flex items-center gap-4 cursor-pointer">
            ...
            <span className="text-xs font-bold text-pfuma-green hover:underline">{photoPreview ? 'Change photo' : 'Upload a photo'}</span>
            <input id="listing-photo" type="file" accept="image/*" className="hidden" onChange={handlePhotoChange} />
          </label>
        </div>
        <button type="submit" disabled={submitting} className="w-full py-3.5 bg-pfuma-green text-white rounded-xl font-bold text-xs uppercase tracking-wide hover:bg-green-700 transition shadow-md flex items-center justify-center gap-2 disabled:opacity-50">
...
const Marketplace = ({ currentUser, animals = [], onListAnimal, presetAnimalId, onPresetConsumed, openForm, onOpenFormConsumed }) => {
  ...
  return (
    <div className="bg-ivory min-h-full text-left overflow-y-auto">
      ...
      <Hero
        ...
        actions={
          <>
            {canPost && (
              <Button variant="onImage" icon={Plus} onClick={() => setShowForm(p => !p)}>
                Post a listing
              </Button>
            )}
            <span className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-xs font-semibold backdrop-blur-sm ${
              apiOnline
                ? 'border-green-300/40 bg-green-950/30 text-green-100'
                : 'border-amber-300/40 bg-amber-950/35 text-amber-100'
            }`}>
              {apiOnline
                ? <><CheckCircle size={13} aria-hidden="true" /> Live — connected to API</>
                : <><AlertTriangle size={13} aria-hidden="true" /> Demo mode — start the Flask API to go live</>}
            </span>
          </>
        }
      />
      ...
      {canPost && (
        <div className="flex gap-1 bg-white border border-gray-100 rounded-2xl p-1 shadow-sm w-fit">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wide transition ${viewMode === 'market' ? 'bg-pfuma-green text-white' : 'text-gray-400 hover:text-gray-600'}`}
          >
            <ShoppingCart size={13} /> All Listings
          </button>
          <button
            onClick={() => setViewMode('mine')}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wide transition ${viewMode === 'mine' ? 'bg-pfuma-green text-white' : 'text-gray-400 hover:text-gray-600'}`}
          >
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <button onClick={fetchListings} className="flex items-center gap-2 px-4 py-3 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-600 uppercase hover:bg-gray-50 transition shadow-sm shrink-0">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
        {CATEGORIES.map(cat => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide shrink-0 transition ${
              activeCategory === cat.id ? `${cat.color} text-white shadow-md` : 'bg-white text-gray-500 border border-gray-100 hover:border-gray-300'
            }`}
          >
            <cat.icon size={13} />
            {cat.label}
            {activeCategory === cat.id && <span className="bg-white/20 px-1.5 py-0.5 rounded-full text-xs">{filtered.length}</span>}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl py-16 text-center">
          ...
          <p className="text-sm font-bold text-gray-400">No listings found</p>
          <p className="text-xs text-gray-300 font-medium mt-1">{canPost ? 'Be the first to post in this category.' : 'Check back soon.'}</p>
          {canPost && <button onClick={() => setShowForm(true)} className="mt-4 px-5 py-2.5 bg-pfuma-green text-white rounded-xl text-xs font-bold uppercase hover:bg-green-700 transition">Post a Listing</button>}
        </div>
      ) : (
            ...
            <div key={listing.id} className={`bg-white rounded-2xl border-2 shadow-sm hover:shadow-md transition flex flex-col overflow-hidden ${CAT_COLORS[listing.category] || 'border-gray-100'}`}>
              ...
              <div className="p-5 flex-1">
                <div className="flex items-start justify-between gap-2 mb-3">
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-full uppercase tracking-wide ${CAT_BADGE[listing.category] || 'bg-gray-100 text-gray-600'}`}>
                    {listing.category}
                  </span>
                  {viewMode === 'mine' && STATUS_BADGE[listing.status] ? (
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-full uppercase tracking-wide ${STATUS_BADGE[listing.status].cls}`}>
                      {STATUS_BADGE[listing.status].label}
                    </span>
                  ) : listing.category === 'livestock' && listing.status !== 'pending_clearance' && (
                    <span className="flex items-center gap-1 text-xs font-bold text-yellow-700 bg-yellow-100 px-2 py-0.5 rounded-full uppercase" title="This livestock listing was reviewed and cleared by Police before going live">
                      <ShieldCheck size={9} /> Police Cleared
                    </span>
                  )}
                </div>

                <h4 className="text-sm font-bold text-gray-900 mb-1 leading-snug">{listing.product_name}</h4>
                {listing.description && (
                  <p className="text-xs text-gray-500 font-medium leading-snug mb-3 line-clamp-2">{listing.description}</p>
                )}
              ...
              <div className="px-5 pb-5 pt-3 border-t border-gray-100 flex items-center justify-between gap-3">
                ...
                <div className="flex gap-2 shrink-0">
                  {listing.user_id === currentUser?.id ? (
                    ...
                    <button onClick={() => handleOrder(listing)} className="flex items-center gap-1.5 px-4 py-2 bg-pfuma-green text-white rounded-xl text-xs font-bold uppercase hover:bg-green-700 transition shadow-sm">
                      <ArrowRight size={12} /> Order
                    </button>
                  ) : (
                    <button onClick={() => handleBid(listing)} className="flex items-center gap-1.5 px-4 py-2 bg-pfuma-green text-white rounded-xl text-xs font-bold uppercase hover:bg-green-700 transition shadow-sm">
                      <ArrowRight size={12} />
              ...
              {listing.user_id === currentUser?.id && openBidsId === listing.id && (
                <div className="px-5 pb-5 pt-1 border-t border-gray-100 space-y-2">
                  {(bidsByListing[listing.id] || []).length === 0 ? (
                    <p className="text-xs text-gray-400 italic font-medium py-2">No bids yet on this listing.</p>
                  ) : (
